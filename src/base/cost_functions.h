...
#ifndef VLOCTIO_COST_FUNCTION_H
#define VLOCTIO_COST_FUNCTION_H
...
// Uncertainty bundle adjustment cost function for variable
// camera pose and calibration and point parameters.
template <typename CameraModel>
class BundleAdjustmentCostFunctionWithPrior {
public:
    explicit BundleAdjustmentCostFunctionWithPrior(const Eigen::Vector2d& point2D)
            : observed_x_(point2D(0)), observed_y_(point2D(1)) {}

    static ceres::CostFunction* Create(const Eigen::Vector2d& point2D) {
        return (new ceres::AutoDiffCostFunction<
                BundleAdjustmentCostFunctionWithPrior<CameraModel>, 2, 4, 3, 3,
                CameraModel::kNumParams, 1>(
                new BundleAdjustmentCostFunctionWithPrior(point2D)));
    }

    template <typename T>
    bool operator()(const T* const qvec, const T* const tvec,
                    const T* const point3D, const T* const camera_params, const T* prior,
                    T* residuals) const {
        // Rotate and translate.
        T projection[3];
        ceres::UnitQuaternionRotatePoint(qvec, point3D, projection);
